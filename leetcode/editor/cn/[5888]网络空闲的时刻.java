...
class Solution {
	...
	public int networkBecomesIdle(int[][] edges, int[] patience) {
		...
		// 构建邻接矩阵
		for (int[] edge : edges) {
			// 表示 起点为 edge[0]，终点为 edge[1] 的链路的距离为 1
			adjacents.get(edge[0]).add(new int[]{edge[1], 1});
			adjacents.get(edge[1]).add(new int[]{edge[0], 1});
		}
		...
		while (!queue.isEmpty()) {
			int[] now = queue.poll();
			if (distances[now[0]] != INF) {
				continue;
			}
			distances[now[0]] = now[1];
			for (int[] next : adjacents.get(now[0])) {
				queue.offer(new int[]{next[0], next[1] + now[1]});
			}
		}

		int res = 0;
		for (int i = 1; i < n; i++) {
			int need = distances[i] * 2;
			if (patience[i] >= need) {
				res = Math.max(res, need);
			}
			else {
				int t = need / patience[i];
				if (need % patience[i] != 0) {
					res = Math.max(res, need + patience[i] * t);
				}
				else {
					res = Math.max(res, need + patience[i] * (t - 1));
				}
			}
		}
